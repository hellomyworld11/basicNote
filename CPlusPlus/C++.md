...
```
当类定义了虚函数，则会产生一个虚函数表，类会包含一个虚函数表指针。 在类的首地址，占用指针大小字节。
```
...
function<int(int)> f = fun;
f(3)
    
bind将function和 参数绑定在一起构成一个新函数。 绑定时用 placeholders::_1来占位不需要绑定的参数。
```

万能引用 完美转发

```
必须是函数模板，并且是T&&形式  ， 实参可以传左值和右值
template<typname T>
void fun(T && a) // 
{
...
```
auto fun  = [](){  return 1; }   ()为空时可省略。  返回类型也可以自己推导。

关于捕获:
	[] 不捕获。
	[&] 捕获外部作用域所有变量，并可作为引用在函数体内使用。 修改会影响外面的值
	[=]  捕获外部作用域所有变量，并作为副本使用。修改了不会影响外面的值
	[this] 捕获当前类的this指针，可以访问类的函数和变量。
	
	[] 里也可以指定只捕获外部哪些变量。
	
mutable 修改lambda表示可以修改外部值。 不管捕获是哪种类型。
```
...
```
空类对象所占用内存为 1字节。 因为对象定义出来都会占用内存，而且支持被访问，而内存取址最小单位为1字节。内存都有首地址，地址为1字节，所以空类对象为1字节。

成员函数不占用对象空间

静态成员和函数不占用对象空间。

如果类有虚函数，那么因为会产生虚函数表指针，所以空间会加上这个指针大小。
```
